...
| Conformance test suite (Python adapters) | ✅ | `tests/test_adapter_conformance.py` runs the same canonical event stream + lifecycle assertions across every shipped adapter (61 tests; 10 invariants × 6 adapters + registry sentinel). New adapters add one line to `ADAPTER_CLASSES` and inherit the entire matrix. |
...
| AWS Bedrock Agents adapter (Python)   |   4   | `pip install agentegrity[bedrock-agents]`. |
| Agno adapter (Python)                 |   4   | `pip install agentegrity[agno]`. |
