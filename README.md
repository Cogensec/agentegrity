...
<a href="pyproject.toml"><img src="https://img.shields.io/badge/library-v0.9.0-green.svg" alt="Library Version"></a>
...
**What it does.** It provides a Python implementation of the four-layer verification architecture defined in the [Agentegrity Specification](spec/SPECIFICATION.md). It computes integrity scores from real evaluation runs, generates cryptographically signed attestation records, builds tamper-evident attestation chains, and produces structured audit logs for governance workflows. Evaluation runs locally with zero required dependencies: no agent content, prompt, tool argument, or attestation ever leaves your process. As of v0.9.0 the library does make one kind of outbound call, and only one: anonymous, shape-only usage telemetry (enum values, counts, rounded scores), documented event by event in [docs/telemetry.md](docs/telemetry.md) and disabled by `DO_NOT_TRACK=1`. See [Telemetry](#telemetry-and-how-to-opt-out) below. It ships with extension points for custom threat detectors, custom policy rules, and custom validators.
